...
export function memorySnapshot(): string {
  ...
  const rows = app.getAppMetrics().map(m => {
    ...
    return { name, pid: m.pid, ws: m.memory.workingSetSize, priv: p }
  }).sort((a, b) => (b.priv - a.priv) || (b.ws - a.ws))

  const heapUsed = (process.memoryUsage().heapUsed / 1048576).toFixed(1)
  const lines = rows.map(r =>
    `  ${r.name.padEnd(40)} pid=${String(r.pid).padEnd(6)} ws=${mb(r.ws).padStart(6)}MB  private=${mb(r.priv).padStart(6)}MB`)
  return [
    `total ws=${mb(ws)}MB private=${mb(priv)}MB across ${rows.length} processes (main heapUsed=${heapUsed}MB)`,
    ...lines,
  ].join('\n')
}
